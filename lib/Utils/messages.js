...
const generateWAMessageContent = async (message, options) => {
    ...
    const uploadFn = options.upload || options.waUploadToServer
    if (!uploadFn) {
        throw new Error('Upload function not available in options')
    }

    const stickerData = {}
    const stickerPromises = stickers.map(async (s, i) => {
        const { encWriteStream: buffer } = await messages_media_1.prepareStream(s.sticker, 'sticker', { 
    ...
    const { encWriteStream: coverBuffer } = await messages_media_1.prepareStream(cover, 'image', { 
        logger: options.logger,
        opts: options.options 
    })
    ...
    const stickerPackUploadResult = await uploadFn(stickerPackUpload.encFilePath, {
