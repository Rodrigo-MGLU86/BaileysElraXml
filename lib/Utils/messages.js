...
const prepareAlbumMessageContent = async (jid, albums, options) => {
    ...
    for (const media of albums) {
        ...
        if ('image' in media) {
            mediaMsg = await (0, exports.generateWAMessage)(jid, {
                image: media.image,
                ...media
            }, {
                userJid: options.userJid,
                messageId: (0, generics_1.generateMessageIDV2)(),
                upload: async (encWriteStream, opts) => {
                    const up = await options.sock.waUploadToServer(encWriteStream, { ...opts, newsletter: (0, WABinary_1.isJidNewsletter)(jid) });
                    return up;
                },
                ...options
            });
